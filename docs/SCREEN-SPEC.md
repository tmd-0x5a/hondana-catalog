...
- 本のパックは未開封時に封をしたパックを表示し、押すと5枚のカードが裏向きで並ぶ。カードをクリックするとその1枚だけが表へ返る。見出し右の「すべてめくる」で一度に開くこともできる。
- カード表面は表紙、選ばれた理由、書名、著者、出版社・出版年、紹介文（取得できた場合）、NDL書誌リンク、本棚追加操作を表示する。
- レア枠のカードは枠と背景を変え、「レア」バッジを付ける。裏面にも光る縁取りを付けてめくる前から特別さが分かるようにする。
...
- `prefers-reduced-motion`が指定された環境では、めくる回転をやめて即座に切り替える。
