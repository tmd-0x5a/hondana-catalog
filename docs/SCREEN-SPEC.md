...
| PC-07 | 本のパック | Windows | 1日1回のカードパックを開封し、未所持候補を表示・登録 |
...
flowchart TD
  ...
  Series -->|巻を選択| Shelf
  Shelf -->|本のパック| Recommend["PC-07 本のパック"]
  Recommend -->|カードから追加| Shelf
...
- 左サイドバー: すべて、新刊、リマインダー、実本、電子書籍・媒体、カテゴリ、iPhone連携QR、蔵書データの書き出しリンク。
...
- 電子書籍のスクリーンショット方式では、最初にストア選択（主要6ストアのボタンとその他の選択）を表示し、選択後に画像選択へ進む。
- スクリーンショットは最大12枚を複数選択でき、OCR後はチェック、表紙、書名、著者、出版年、代替候補を一覧表示する。ISBN・表紙を取得できない行は「書名のみ」と案内し、そのまま選択登録できる。
...
- 本のパックは書店の紙封筒を表示する。封筒の口を上へドラッグして開き、続けてカード束を上へ引き抜くと、5枚が時間差で裏向きに並ぶ。カードをクリックするとその1枚だけが表へ返り、見出し右の「すべてめくる」でも一度に開ける。
- カード表面は表紙、選ばれた理由、書名、著者、出版社・出版年、紹介文（取得できた場合）、NDL書誌リンク、本棚追加操作を表示する。
- 裏面は全カードで同じ深緑の装丁とし、レア枠を事前に判別させない。レア枠をめくった後だけ真鍮色の発光、スパーク、選定理由を重ね、表面へ「レア」バッジを表示する。
- 準備中は進捗バーと「何冊ぶんの候補が見つかったか」を表示し、他の画面へ移動してよい旨を伝える。完成すると自動で切り替わる。
- 見出しの説明文は未開封・めくり途中・全部めくり終わりで切り替え、残り枚数と次のパックが明日であることを案内する。
- めくった札は日付ごとに端末へ記録し、再訪時は続きからめくれる。
- `prefers-reduced-motion`が指定された環境では、封筒、カード配置、レア演出、めくる回転を停止して即座に切り替える。
