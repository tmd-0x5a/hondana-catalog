import { Archive, FileUp, Images, Info, List, RefreshCw, ScanText, Smartphone, Upload, X } from "lucide-react";
import { useMemo, useState } from "react";

import { requestJson } from "../api.js";
import { parseBulkImportText } from "../bulk-import-model.js";
import { showFallbackCover } from "../cover-image.js";
import { PLATFORM_CATALOG, PLATFORM_OPTIONS } from "../library-model.js";

const FEATURED_PLATFORMS = PLATFORM_CATALOG.filter((platform) => platform.featured).map((platform) => platform.name);
const OTHER_PLATFORMS = PLATFORM_OPTIONS.filter((platform) => !FEATURED_PLATFORMS.includes(platform));
...
export function BulkImportModal({ initialFormat = "physical", onClose, onImported }) {
  ...
  // スクリーンショット取り込みでは、登録先ストアの選び忘れを防ぐため先にストアを確定させる。
  ...
  const selectedEntries = useMemo(() => {
    ...
    return scanRows.flatMap((row) => {
      ...
      const bookKey = suggestion?.isbn || `title:${suggestion?.title?.normalize("NFKC").toLocaleLowerCase("ja")}`;
      if (!row.selected || !suggestion?.title || seenBooks.has(bookKey)) return [];
  ...
  const requiresStoreChoice = format === "electronic" && sourceMode === "screenshots" && !platformConfirmed;
  ...
  function confirmPlatform(platform) {
    if (!platform) return;
    setElectronicPlatform(platform);
  ...
  async function scanScreenshots(event) {
    ...
    try {
      ...
      const defaultBooks = new Set();
      setScanRows(response.candidates.map((row, index) => {
        const firstSuggestion = row.suggestions[0];
        const bookKey = firstSuggestion?.isbn || `title:${firstSuggestion?.title?.normalize("NFKC").toLocaleLowerCase("ja")}`;
        const selected = Boolean(firstSuggestion?.title) && !defaultBooks.has(bookKey);
        if (selected) defaultBooks.add(bookKey);
      ...
      <form className="edit-modal bulk-import-modal" onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}>
        ...
        <div className="bulk-target">
          {format === "physical" ? (
            <label>
              <span>保管場所</span>
              <input maxLength={300} onChange={(event) => setPhysicalLocation(event.target.value)} value={physicalLocation} />
            </label>
          ) : !requiresStoreChoice && (
            <label>
              <span>電子書籍ストア</span>
              <select onChange={(event) => confirmPlatform(event.target.value)} value={electronicPlatform}>
        ...
        {sourceMode === "screenshots" ? (
          <>
            {requiresStoreChoice ? (
              <section className="store-choice" aria-label="ストア選択">
                <strong>どこのスクリーンショットですか？</strong>
                <span>選んだストアが登録先の電子書籍媒体になります。</span>
                <div className="store-choice-grid">
                  {FEATURED_PLATFORMS.map((platform) => (
                    <button key={platform} onClick={() => confirmPlatform(platform)} type="button">{platform}</button>
                  ))}
                </div>
                <label className="store-choice-other">
                  <span>その他のストア・媒体</span>
                  <select onChange={(event) => confirmPlatform(event.target.value)} value="">
                    <option value="">選択してください</option>
                    {OTHER_PLATFORMS.map((platform) => <option key={platform}>{platform}</option>)}
                  </select>
                </label>
              </section>
            ) : (
              ...
              <div className="scan-candidate-list">
                {scanRows.map((row) => {
                  ...
                  return <div className={`scan-candidate ${row.selected ? "selected" : ""}`} key={row.id}>
                    ...
                    <div className="scan-book-copy"><strong>{suggestion.title}</strong><span>{suggestion.metadataPending ? "書名のみで登録（ISBN・表紙は未取得）" : [suggestion.author, suggestion.published].filter(Boolean).join(" / ") || "著者情報なし"}</span><small>画像内: {row.sourceText}</small></div>
        ...
        {notice && <div className="bulk-notice"><Info size={15} /><span>{notice}</span></div>}
