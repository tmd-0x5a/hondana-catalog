...
- 電子書店のスクリーンショットを最大12枚選び、座標付きWindows OCRと書誌候補確認から一括取り込み
...
- 起動日ごとの蔵書スナップショット自動保存（最新7世代）と、蔵書全件のJSON書き出し
...
3. 電子書店のライブラリ画面のスクリーンショットを最大12枚選びます。Windows内蔵OCRが文字の位置と大きさから書名行だけを抽出し、まとめてNDLサーチへ照合します。
4. 候補の書名、著者、出版年、表紙を確認し、必要な本だけチェックして取り込みます。ISBNや表紙を取得できない本も、OCR書名だけで登録して後から編集できます。
...
- 起動日ごとに蔵書一覧のスナップショットを `data\backups` へ自動保存し、最新7世代を残します。誤って削除・上書きした場合は、該当日の `books-YYYY-MM-DD.json` を `books.json` へ戻して復旧できます。
- PC画面左下の「蔵書データを書き出す」から、蔵書全件をJSONファイルとしてダウンロードできます。
...
- 国立国会図書館サーチ: タイトル検索、新刊候補、おすすめ候補、表紙画像（書影）のフォールバック
...
- スクリーンショット画像はWindows内蔵OCRでPC内処理し、外部へ送信しません。OCRから抽出した書名候補（1枚あたり最大12件・全体最大60件）だけを、10件ずつの一括検索としてNDLサーチへ送信します。
...
- JSON破損に備えた直前一世代の`.bak`と、起動日ごとの蔵書スナップショット（最新7世代）をPC内へ作成します。PC故障に備えた外部バックアップは行わないため、必要に応じて`data`フォルダーまたは書き出したJSONを別媒体へ保存してください。
